...
params['solver']['idx_e'] = [6, 7]
looper = wrap_looper(SystemClass=Uni01, params=params, func='aes', looper='x_looper', file_path='data/uni_00/aes_1e4-20pi')
looper.results['V'] = [np.max(np.real(v)) for v in looper.results['V']]
print(looper.get_thresholds(thres_mode='minmax'))
...
# right axis for average phase synchronization
...
ax.plot(X, M_1, linestyle='-', color=_colors[-3])
ax.scatter(X, M_1, marker='o', color=_colors[-3], s=15)
plotter.show(True)